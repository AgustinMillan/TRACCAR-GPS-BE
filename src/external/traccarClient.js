...
class TraccarClient {
  ...
  async getPositions(trackingToken) {
    try {
      ...
      const latitude =
        String(data.latitude).split(".")[0] +
        "." +
        String(data.latitude).split(".")[1].slice(0, 6);
      const longitude =
        String(data.longitude).split(".")[0] +
        "." +
        String(data.longitude).split(".")[1].slice(0, 6);

      return {
        latitude: latitude,
        longitude: longitude,
        speed: String(data.speed).split(".")[0],
        date: data.fixTime,
      };
    } catch (error) {
      throw new Error(
        `Error obteniendo posiciones del dispositivo ${trackingToken}: ${error.message}`
      );
    }
  }
}
